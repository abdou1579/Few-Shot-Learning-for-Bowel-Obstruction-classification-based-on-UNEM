...
class Evaluator_few_shot:
    ...
    def extract_and_load_features(self, model, dataset, data_loaders):
        ...
        if hasattr(self.args, 'use_pre_softmax_feature') and self.args.use_pre_softmax_feature:
            ...
            filepath_support = 'data/{}/saved_features/train_pre_softmax_{}.plk'.format(
    ...
    def get_method_builder(self, model, device, args, log_file):
        ...
        if args.name_method == 'EM_DIRICHLET':
        ...
        elif args.name_method == 'U_EM_DIRICHLET':
            method_info.pop('model')
            method_builder = U_EM_DIRICHLET(**method_info)
            path = 'u_params/{}/{}/{}'.format(self.args.dataset, self.args.backbone, self.args.iter)
            checkpoint_file = path + '/{}_{}shots.pt'.format(self.args.name_method, self.args.shots)
            if os.path.isfile(checkpoint_file) == True:
                ...
                raise ValueError('Checkpoint file for unrolled version not found')
            
        elif args.name_method == 'U_EM_DIRICHLET_L':
            method_info.pop('model')
            method_builder = U_EM_DIRICHLET(**method_info, diff_gamma_layers=True)
            path = 'u_params/{}/{}/{}'.format(self.args.dataset, self.args.backbone, self.args.iter)
            checkpoint_file = path + '/{}_{}shots.pt'.format(self.args.name_method, self.args.shots)
            if os.path.isfile(checkpoint_file) == True:
                print(f"Loading checkpoint from {checkpoint_file}")
            ...
            method_info.pop('model')
            method_builder = U_EM_DIRICHLET_EMBT(**method_info, diff_gamma_layers=True)
            path = 'u_params/{}/{}/{}'.format(self.args.dataset, self.args.backbone, self.args.iter)
            checkpoint_file = path + '/{}_{}shots.pt'.format(self.args.name_method, self.args.shots)
            if os.path.isfile(checkpoint_file) == True:
                print(f"Loading checkpoint from {checkpoint_file}")
            ...
            method_info.pop('model')
            method_builder = U_EM_DIRICHLET_EMBT(**method_info)
            path = 'u_params/{}/{}/{}'.format(self.args.dataset, self.args.backbone, self.args.iter)
            checkpoint_file = path + '/{}_{}shots.pt'.format(self.args.name_method, self.args.shots)
            if os.path.isfile(checkpoint_file) == True:
                print(f"Loading checkpoint from {checkpoint_file}")
        ...
        elif args.name_method == 'U_EM_DIRICHLET_EMBT_L_AVG':
            method_info.pop('model')
            method_builder = U_EM_DIRICHLET_EMBT(**method_info, diff_gamma_layers=True)
            path = 'u_params/average/'
            checkpoint_file = path + 'U_EM_DIRICHLET_EMBT_L_4shots.pt'
            if os.path.isfile(checkpoint_file) == True:
                print(f"Loading checkpoint from {checkpoint_file}")
                initial_state_dict = copy.deepcopy(method_builder.state_dict())
                method_builder.load_state_dict(torch.load(checkpoint_file))
    ...
    def evaluate_tasks(self, model, all_features_support, all_labels_support, all_features_query, all_labels_query):
        ...
        for i in tqdm(range(int(self.args.number_tasks/self.args.batch_size)), desc='Tasks generation'):
            ...
            try:
                self.logger.info("runnung with params: ")
                for key, value in method.named_parameters():
    ...
    def report_results(self, mean_accuracies, mean_times):
        ...
        if self.args.acc_vs_gamma_study:
            import pandas as pd
            data = {
                    "lambda": [self.args.lamda],
                    "accuracy": [mean_accuracies],
                }
            df = pd.DataFrame(data)
            # read a csv and append the results
            dir = f'plot/'
            path =  '{}_plot.csv'.format(self.args.dataset)
