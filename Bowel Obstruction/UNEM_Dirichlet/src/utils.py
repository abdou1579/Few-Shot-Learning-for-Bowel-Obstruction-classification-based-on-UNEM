...
def extract_features_pre_softmax(model, dataset, loader, set_name, args,
                             device, list_T=[10, 20, 30, 40, 50]):
    """
        inputs:
            model : The loaded model containing the feature extractor
        ...
        returns :
            Saves the features in data/args.dataset/saved_features/ 
    """
 
    # Check if features are already saved
    features_save_path = 'data/{}/saved_features/{}_pre_softmax_{}.plk'.format(
        args.dataset, set_name, args.backbone)
    if os.path.exists(features_save_path):
        print('Features already saved for {} set pre-softmax, skipping'.format(set_name))
        return
    else:
        print('Extracting {} features on {} set pre-softmax'.format(set_name, args.dataset))
