...
class KM(object):
    ...
    def run_task(self, task_dic, shot):
        ...
        support = support.to(self.device)                # [n_task, shot, feature_dim]
        query = query.to(self.device)                    # [n_task, n_query, feature_dim]
        y_s = y_s.long().squeeze(2).to(self.device)      # [n_task, shot*way]
        y_q = y_q.long().squeeze(2).to(self.device)      # [n_task, shot]
...
class PADDLE(KM):
    ...
    def v_update(self):
        ...
        self.v = torch.log(self.A(self.u) + 1e-6) 
