...
class Baseline(object):
    ...
    def run_method(self, support, query, y_s, y_q):
        ...
        if self.iter == 0:
            t1 = time.time()
            self.model.eval()
            self.record_info(new_time=t1, query=query, y_q=y_q)
        else:
            self.logger.info(" ==> Executing Baseline adaptation over {} iterations on {} shot tasks...")
            ...
            self.model.eval()
            self.record_info(new_time=1, query=query, y_q=y_q)
